...
import { insertQueuedOperations } from './utils';
...
export default async (nodes, fromWatcher) => {
  ...
  if (!currentChain) {
    ...
    if (hasNodeDied) {
      ...
      const nodeBeforeFailedNode =
        changedNodeKey > 2 ? currentChain[changedNodeKey - 1] : chain.head;

      if (!isHead && !isTail) {
        range(changedNodeKey + 1, highestBeforeTail + 1).forEach(async r => {
          await insertQueuedOperations(nodeBeforeFailedNode, currentChain[r]);
        });

        await insertQueuedOperations(nodeBeforeFailedNode, chain.tail);
      }
      ...
      await insertQueuedOperations(chain.tail, recoveredNode.address);
